 ...
 */
class pdf_davila extends ModelePDFPropales
{
	/**
	...
	public function __construct($db)
	{
		...
		$this->db = $db;
		$this->name = "davila";
		$this->description = $langs->trans('DocModelDavilaDescription');
	...
	protected function _pagehead(&$pdf, $object, $showaddress, $outputlangs)
	{
		...
		if (empty($conf->global->PDF_DISABLE_MYCOMPANY_LOGO))
		{
			if ($this->emetteur->logo)
			{
				...
				if (is_readable($logo))
				{
					$height = pdf_getHeightForLogo($logo) * 2;
					$html_logo = '<td><img src="'.$logo.'" height="'.$height.'"></td>';  
				}
				else
				{

					$html_logo = '<td><p>'.$outputlangs->transnoentities("ErrorLogoFileNotFound", $logo).'</p></td>';
					$html_logo = '<td><p>'.$outputlangs->transnoentities("ErrorGoToGlobalSetup").'</p></td>';
				}
			}
			else
			{
				$text = $this->emetteur->name;
				$html_text = '<td><p>'.$text.'</p></td>';
				
			}
		}

	
		$html_text = '<td>'.$outputlangs->convToOutputCharset($this->emetteur->name).'</td>';
		// $html_text .= '<td align="right"><p>'.$outputlangs->transnoentities("PdfCommercialProposalTitle").'</p>';
		// $html_text .= '<span>'.$outputlangs->transnoentities("Ref")." : ".$outputlangs->convToOutputCharset($object->ref).'</span>';
		// $html_text .= '</td>';
